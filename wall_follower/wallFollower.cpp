...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    ...
    if (move_type == STOP) {
        ...
        motor_command.angular.z = 0.0;
        motor_command.linear.x = 0.5*scale;
    }

    else if (move_type == BACKWARD) {
        ROS_INFO("[ROBOT] I'm going back! \n");
        motor_command.linear.x = -0.75*scale;
        motor_command.angular.z = 0.0;
    }
        ...
        motor_command.linear.x = 0.0;
        motor_command.angular.z = 1.0*scale;
    }
        ...
        motor_command.linear.x = 0.25*scale;
        motor_command.angular.z = -0.25*scale;
    }
